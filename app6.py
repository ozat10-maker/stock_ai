            ...
            if not api_key: st.warning("אנא הזן מפתח API ב-Sidebar")
                ...
                if comparison_data:
                    ...
                    for c in comparison_data:
                        comp_desc += f"- מניית {c['מניה']}: סקטור {c['סקטור']}, בטא {c['בטא']:.2f}, אנליסטים: {c['דירוג אנליסטים']}. חדשות: {c['חדשות']}.\n"
                    ...
                    try:
                        with st.spinner("ממתין סנכרון שרת למניעת עומס (429)..."):
                            time.sleep(3) # הגנה קשיחה למניעת חסימת ה-Rate Limit בהשוואת נכסים
