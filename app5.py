        ...
        if st.button("נתח מניה והפק המלצה"):
            if not api_key: st.warning("אנא הזן מפתח API ב-Sidebar")
                ...
                if data:
                    ...
                    אתה אנליסט פיננסי ומנהל סיכונים בכיר. המשתמש פועל תחת פרופיל סיכון {risk_profile}.
                    עליך לבצע בדיקה מפוצלת עבור הנכס ולהציג את הפלט במבנה הבא בדיוק, תוך הפרדה מוחלטת:
                    
                    ---
                    ### 📂 ניתוח 1: התאמה לתיק ורמת סיכון (Portfolio Risk Fit)
                    בניתוח זה התייחס אך ורק לתיק ההשקעות הקיים ולפרופיל הסיכון של המשתמש. בדוק את חוקי הריכוזיות היבשים (Conservative: 15%, Moderate: 25%, Aggressive: 45%), שקלל את מדד הבטא (אם Beta > 1.5 החמר את הרף ב-5%), ובדוק אם יש ריכוזיות יתר בסקטור {data['sector']}. הצג אזהרת ⚠️ אם יש חריגה וחשב סכום צימוד מותר.
                    
                    ---
                    ### 📈 ניתוח 2: הערכת המניה עצמה ותזמון טקטי (Asset Value & Timing)
                    בניתוח זה התעלם לחלוטין מחוקי הריכוזיות של התיק ומהפרופיל של המשתמש. נתח את המניה כנכס עצמאי על בסיס: המרחק שלה מהשיא ({data['pct_from_high']:.1f}%), מחיר היעד של האנליסטים, המגמה הטכנית, והסנטימנט העולמי בחדשות.
                    ספק המלצה חד-משמעית ומנומקת מבין 3 האפשרויות הבאות:
                    1. **לקנות כעת** (כי הנכס במגמה חזקה וסנטימנט מעולה).
                    2. **לחכות למחיר זול יותר כדי לשפר פוזיציה** (למשל, אם המניה כרגע בשיא מתוח או חווה ירידות בריאות של 'מכירה בחדשות' לאחר דוחות, ומומלץ להמתין או לפרוס למנות).
                    3. **לא לקנות בכלל במועד הנתון** (אם המניה במגמה שלילית חריפה, מכפיל יקר מדי באופן קיצוני או חדשות מדאיגות).
                    
                    ---
                    פורמט פלט: {lang_instruction} {format_instruction}
                    """
            ...
            if not api_key: st.warning("אנא הזן מפתח API ב-Sidebar")
                ...
                if comparison_data:
                    ...
                    for c in comparison_data:
                        comp_desc += f"- מניית {c['מניה']}: סקטור {c['סקטור']}, בטא {c['בטא']:.2f}, אנליסטים: {c['דירוג אנליסטים']}, מרחק מהשיא {c['מרחק משיא']}. חדשות: {c['חדשות']}.\n"
                    ...
                    try:
                        client = genai.Client(api_key=api_key)
                        response = client.models.generate_content(model='gemini-2.5-flash', contents=user_context, config=types.GenerateContentConfig(system_instruction=system_instruction, temperature=0.3))
                        st.write("---")
