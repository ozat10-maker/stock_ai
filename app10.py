...
if "GEMINI_API_KEY" in st.secrets:
    ...
    st.sidebar.success("✔️ מפתח API נטען אוטומטית מהשרת")
else:
...
def fetch_stock_advanced_engine(ticker_str):
    try:
        ...
        return {
            ...
            "analyst_rating": recommendation, "news": news_summary,
...
if st.session_state.page == "setup":
    ...
    if st.button("הוסף מניה לתיק"):
        if new_ticker:
            ...
            if data:
                ...
                st.rerun()
            else: st.error("❌ סימול לא נמצא ב-Yahoo Finance או שיש בעיית רשת זמנית.")
    ...
    short_fmt = "Provide a SHORT, ultra-concise report. Ensure both Analysis #1 and Analysis #2 are strictly answered in 2 sentences each max. State only the hard facts and the buy/hold/sell timing consensus."
    full_fmt = "Provide a FULL, comprehensive investment report. Deeply expand both Analysis #1 and Analysis #2 with mathematical justification, sector correlation, news details, and advanced entry strategies."

    if mode == "🧐 בחינת מניה בודדת כפוטנציאל השקעה":
        target_ticker = st.text_input("הזן סימול מניה לבחינה (למשל NVDA):", "NVDA").upper().strip()
        
        if st.button("נתח מניה (דוח קצר ומהיר)"):
            if not api_key: st.warning("אנא הזן מפתח API ב-Sidebar")
            elif target_ticker:
                with st.spinner("מפיק דוח קצר וקולע בזריזות..."):
                ...
                if data:
                    ...
                    user_context = f"[מצב תיק קיים]\nשווי כולל: \${st.session_state.total_portfolio_value:.2f}\nפירוט נכסים:\n{portfolio_desc}\n\n[העסקה המבוקשת]\nהמשתמש רוצה להשקיע סכום חדש של \${additional_capital} במניית {target_ticker}.\nנתוני שוק: סקטור {data['sector']}, בטא {data['beta']:.2f}, מכפיל {data['pe']}, מגמה {data['trend']}.\nמרחק מהשיא: {data['pct_from_high']:.1f}%.\nאנליסטים: קונזנזוס {data['analyst_rating']}, מחיר יעד \${data['target_price']}.\nחדשות: {data['news']}.\nחשיפה נוכחית: {current_concen:.1f}%, חשיפה פוטנציאלית סופית: {potential_concen:.1f}%."
                    system_instruction = f"אתה אנליסט פיננסי בכיר. פעל תחת פרופיל {risk_profile}. בצע בדיקה מפוצלת: ### 📂 ניתוח 1: התאמה לתיק ורמת סיכון. ### 📈 ניתוח 2: הערכת המניה עצמה ותזמון טקטי (לקנות כעת, לחכות למחיר זול לשיפור פוזיציה, או לא לקנות בכלל). {lang_instruction} {short_fmt}"
                    ...
                    try:
                        ...
                        st.write("---")
                        st.subheader(f"⚡ תמצית מנהלים מה-AI (קצר וקולע)")
                        st.markdown(response.text)
                        
                        st.session_state.last_context = user_context
                        st.session_state.last_sector_data = data
                    except Exception as e: st.error(f"שגיאה: {str(e)}")
                else: st.error("הסימול לא נמצא בשוק.")
        
        if "last_context" in st.session_state:
            st.write("---")
            st.write("💡 רוצה להעמיק בנתונים, לנתח סנטימנט חדשות עולמי ואסטרטגיות פריסה?")
            if st.button("🔍 הפק דוח מלא ומורחב מה-AI", type="secondary"):
                with st.spinner("מנוע ה-AI מפיק דוח עומק מפורט (זה עשוי לקחת מספר שניות)..."):
                    system_instruction_full = f"אתה אנליסט פיננסי ומנהל סיכונים בכיר. המשתמש פועל תחת פרופיל סיכון {risk_profile}. בצע בדיקה מפוצלת מורחבת ומעמיקה במיוחד: ### 📂 ניתוח 1: התאמה לתיק ורמת סיכון (כלול חישובי ריכוזיות מדויקים, בטא, ומתאם סקטוריאלי). ### 📈 ניתוח 2: הערכת המניה עצמה ותזמון טקטי (ספק נימוק עומק מורחב מבין 3 האפשרויות: לקנות כעת, לחכות למחיר זול יותר לשיפור פוזיציה, או לא לקנות בכלל, תוך שקלול סנטימנט חדשות עולמי מורחב ואסטרטגיות כניסה). {lang_instruction} {full_fmt}"
                    try:
                        client = genai.Client(api_key=api_key)
                        response_full = client.models.generate_content(model='gemini-2.5-flash', contents=st.session_state.last_context, config=types.GenerateContentConfig(system_instruction=system_instruction_full, temperature=0.2))
        ...
        if st.button("בצע השוואה והפק המלצה"):
            if not api_key: st.warning("אנא הזן מפתח API ב-Sidebar")
                ...
                if comparison_data:
                    ...
                    system_instruction = f"אתה מנהל סיכונים בכיר. השווה בין החלופות עבור פרופיל {risk_profile}. שקלל את חוקי הריכוזיות, רמות התנודתיות, והמרחק מהשיא השנתי. ספק סיכום קצר והמלצה ברורה בשורה התחתונה לגבי חלוקת הכסף. {lang_instruction}"
                    ...
                    for attempt in range(3):
                        try:
                            ...
                            if "429" in str(e) and attempt < 2:
                                with st.spinner(f"עומס בשרת גוגל, מבצע ניסיון חוזר אוטומטי בעוד 5 שניות..."):
                                    time.sleep(5)
                            else:
